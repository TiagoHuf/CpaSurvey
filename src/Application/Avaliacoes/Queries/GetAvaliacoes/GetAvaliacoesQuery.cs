...
public class GetAvaliacoesQueryHandler : IRequestHandler<GetAvaliacoesQuery, List<Avaliacao>>
{
    ...
    public async Task<List<Avaliacao>> Handle(GetAvaliacoesQuery request, CancellationToken cancellationToken)
    {
        var repository = _unitOfWork.GetRepository<Avaliacao>();

        var Avaliacaos = await repository
            .GetAll()
